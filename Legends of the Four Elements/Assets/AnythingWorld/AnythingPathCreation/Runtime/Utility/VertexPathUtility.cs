...
{
	/// <summary>
	/// This utility class provides methods for splitting a Bezier path based on angle error or evenly
	/// spacing points along the path.
	/// </summary>
    public static class VertexPathUtility
    {
        /// <summary>
	    /// Splits a Bezier path into segments based on the maximum angle error allowed.
        /// </summary>
			...
			float minVertexDst, float accuracy)
        {
            ...
            splitData.tangents.Add(CubicBezierUtility.EvaluateCurveDerivative(
	            bezierPath.GetPointsInSegment(0), 0).normalized);
            splitData.cumulativeLength.Add(0);
            splitData.anchorVertexMap.Add(0);
			splitData.minMax.UpdateValues(bezierPath[0]);
            ...
            for (int segmentIndex = 0; segmentIndex < bezierPath.NumSegments; segmentIndex++)
            {
                ...
                for (float t = increment; t <= 1; t += increment)
                {
                    ...
                    if ((angleError > maxAngleError && dstSinceLastVertex >= minVertexDst) || isLastPointOnPath)
                    {
						...
						splitData.minMax.UpdateValues(pointOnPath);
        ...
        /// <summary>
		/// Splits a Bezier path into segments with evenly spaced vertices.
        /// </summary>
		...
		/// <summary>
		/// Represents data resulting from splitting a Bezier path.
		/// </summary>
